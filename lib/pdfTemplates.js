...
export const TEMPLATES = {
  template1: {
    ...
    previewEmoji: '📄',
    features: ['מסגרת צבעונית', 'לוגו במרכז', 'עיצוב מסורתי']
  },
  template2: {
    id: 'template2',
    name: 'מודרני',
    ...
    previewEmoji: '✨',
    features: ['עיצוב נקי', 'פסי צבע', 'גרדיאנט מודרני']
  },
  template3: {
    id: 'template3',
    name: 'עסקי',
    ...
    previewEmoji: '💼',
    features: ['סייד-בר צבעוני', 'לוגו עגול', 'מקצועי']
  }
};

export function generatePDFTemplate(templateId, quoteData, businessData, logoUrl, userColor) {
  // Backward compatibility: convert numeric IDs to string IDs
  let normalizedTemplateId = templateId;
  if (typeof templateId === 'number') {
    normalizedTemplateId = `template${templateId}`;
  }

  const template = TEMPLATES[normalizedTemplateId];
  if (!template) {
    console.warn(`Template ${templateId} not found, falling back to template1`);
    normalizedTemplateId = 'template1';
  }
...
function generateTemplate1(quote, business, logoUrl, userColor) {
  ...
  // Format status based on signature_status
  const getStatus = () => {
    if (quote.signature_status === 'signed') return 'נחתם';
    return 'ממתין לחתימה';
  };
    ...
    <head>
      ...
      <style>
        ...
        .section-title {
          ...
          font-size: 16px;
          padding-bottom: 8px;
          border-bottom: 2px solid ${userColor || '#FDDC33'};
        }
        .items-table {
          width: 100%;
    ...
    <body>
      <div class="container">
        ...
        <div class="quote-header">
          <div class="quote-number">הצעת מחיר מספר: ${quote.proposal_number || quote.id}</div>
          <div class="quote-details">
            ...
            <div class="quote-info">
              ...
              <div><strong>תאריך הצעה:</strong> ${formatDate(quote.created_at)}</div>
              <div><strong>תוקף עד:</strong> ${formatDate(validityDate)}</div>
              ${quote.delivery_date ? `<div><strong>תאריך אספקה:</strong> ${formatDate(quote.delivery_date)}</div>` : ''}
              <div><strong>סטטוס:</strong> ${getStatus()}</div>
            </div>
          </div>
        </div>
        ...
        <table class="items-table">
          ...
          <tbody>
            ${items.map(item => `
              <tr>
                <td>
                  ${item.custom_name || item.product_name || item.product?.name || 'פריט'}
                  ${item.notes ? `<br><span style="font-size: 12px; color: #888; font-style: italic;">${item.notes}</span>` : ''}
                </td>
                <td>${item.qty || 1}</td>
        ...
        <div class="totals">
          <div class="total-row">
            ...
            <span>${formatCurrency(items.reduce((sum, item) => sum + ((item.unit_price || 0) * (item.qty || 1)), 0))}</span>
          </div>
          ${(quote.discount_amount > 0 || quote.discount_percentage > 0) ? `
            <div class="total-row">
              <span>הנחה${quote.discount_percentage > 0 ? ` (${quote.discount_percentage}%)` : ''}:</span>
              <span>-${formatCurrency((quote.discount_amount || 0) + (items.reduce((sum, item) => sum + ((item.unit_price || 0) * (item.qty || 1)), 0) * (quote.discount_percentage || 0) / 100))}</span>
            </div>
          ` : ''}
          <div class="total-row">
            <span>לפני מע"ם:</span>
            <span>${formatCurrency((quote.total || 0) / 1.18)}</span>
          </div>
          <div class="total-row">
            <span>מע"ם (18%):</span>
            <span>${formatCurrency((quote.total || 0) - ((quote.total || 0) / 1.18))}</span>
          </div>
...
function generateTemplate2(quote, business, logoUrl, userColor) {
  ...
  // Use the financial totals from the database (already calculated correctly)
  ...
  return `
    ...
    <head>
      ...
      <style>
        @page {
          margin: 20px;
          border: 2px solid ${userColor || '#FDDC33'};
        }
        body {
          font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
          margin: 0;
        ...
        <div class="content">
          <div class="quote-meta">
            <div class="meta-section">
              ...
              ${quote.delivery_date ? `
                <div class="meta-item">
                  <span class="meta-label">תאריך אספקה:</span>
                  <span class="meta-value">${formatDate(quote.delivery_date)}</span>
                </div>
              ` : ''}
              ${quote.title ? `
                <div class="meta-item">
          ...
          <div class="totals-section">
            ...
            ${(quote.discount_amount > 0 || quote.discount_percentage > 0) ? `
              <div class="total-row" style="color: #e74c3c;">
                <span class="total-label">הנחה${quote.discount_percentage > 0 ? ` (${quote.discount_percentage}%)` : ''}:</span>
                <span class="total-value">-${formatCurrency((quote.discount_amount || 0) + (subtotal * (quote.discount_percentage || 0) / 100))}</span>
              </div>
            ` : ''}
            <div class="total-row">
              <span class="total-label">מע"ם (18%):</span>
        ...
        <div class="main-content">
          ...
          <div class="info-grid">
            ...
            <div class="info-card">
              ...
              ${quote.delivery_date ? `
                <div class="info-row">
                  <span class="info-label">תאריך אספקה:</span>
                  <span class="info-value">${formatDate(quote.delivery_date)}</span>
                </div>
              ` : ''}
              ${quote.title ? `
                <div class="info-row">
          ...
          <div class="totals-container">
            ...
            ${(quote.discount_amount > 0 || quote.discount_percentage > 0) ? `
              <div class="total-line" style="color: #e74c3c;">
                <span class="total-label">הנחה${quote.discount_percentage > 0 ? ` (${quote.discount_percentage}%)` : ''}:</span>
                <span class="total-amount">-${formatCurrency((quote.discount_amount || 0) + (subtotal * (quote.discount_percentage || 0) / 100))}</span>
              </div>
            ` : ''}
            <div class="total-line">
              <span class="total-label">מע"ם (18%):</span>
