...
// Load webhook URLs from Expo config (app.json extra) or environment variables
// Note: Webhook URLs should be configured via EAS Secrets for production
const SLACK_WEBHOOKS = {
  user_activity: Constants.expoConfig?.extra?.slackUserActivityWebhookUrl || process.env.SLACK_USER_ACTIVITY_WEBHOOK_URL,
  quote_activity: Constants.expoConfig?.extra?.slackQuoteActivityWebhookUrl || process.env.SLACK_QUOTE_ACTIVITY_WEBHOOK_URL,
  admin_alerts: Constants.expoConfig?.extra?.slackAdminAlertsWebhookUrl || process.env.SLACK_ADMIN_ALERTS_WEBHOOK_URL,
  errors: Constants.expoConfig?.extra?.slackErrorsWebhookUrl || process.env.SLACK_ERRORS_WEBHOOK_URL,
};

// Debug: Log webhook configuration on module load
console.log('[Slack] Webhook URLs configured:', {
  user_activity: !!SLACK_WEBHOOKS.user_activity,
  quote_activity: !!SLACK_WEBHOOKS.quote_activity,
  admin_alerts: !!SLACK_WEBHOOKS.admin_alerts,
  errors: !!SLACK_WEBHOOKS.errors,
});

console.log('[Slack] Full extra config:', Constants.expoConfig?.extra);
