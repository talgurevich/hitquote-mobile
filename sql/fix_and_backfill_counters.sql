-- Fix missing settings and backfill counters for a business
DO $$
DECLARE
  v_business_id uuid := '227b08b4-9039-4bfe-b79d-a4cd7c30491a';
  v_current_month date := DATE_TRUNC('month', CURRENT_DATE)::DATE;
  v_total_count int;
  v_monthly_count int;
  v_business_name text;
  v_business_email text;
BEGIN
  -- Get business info
  SELECT business_name, business_email
  INTO v_business_name, v_business_email
  FROM businesses
  WHERE id = v_business_id;

  RAISE NOTICE 'Business: % (%)', v_business_name, v_business_id;

  -- Count total quotes for this business
  SELECT COUNT(*) INTO v_total_count
  FROM proposal
  WHERE business_id = v_business_id;

  -- Count quotes created this month
  SELECT COUNT(*) INTO v_monthly_count
  FROM proposal
  WHERE business_id = v_business_id
    AND DATE_TRUNC('month', created_at) = v_current_month;

  RAISE NOTICE 'Total quotes: %', v_total_count;
  RAISE NOTICE 'Monthly quotes: %', v_monthly_count;

  -- Create or update settings record
  INSERT INTO settings (
    business_id,
    business_name,
    business_email,
    header_color,
    pdf_template,
    monthly_quotes_created,
    total_quotes_created,
    monthly_counter_reset_date
  ) VALUES (
    v_business_id,
    v_business_name,
    v_business_email,
    '#FDDC33',
    1,
    v_monthly_count,
    v_total_count,
    v_current_month
  )
  ON CONFLICT (business_id)
  DO UPDATE SET
    monthly_quotes_created = v_monthly_count,
    total_quotes_created = v_total_count,
    monthly_counter_reset_date = v_current_month;

  RAISE NOTICE '✅ Settings record created/updated with backfilled counters';
END $$;

-- Verify the result
SELECT
  s.business_id,
  b.business_name,
  s.monthly_quotes_created,
  s.total_quotes_created,
  s.monthly_counter_reset_date
FROM settings s
JOIN businesses b ON b.id = s.business_id
WHERE s.business_id = '227b08b4-9039-4bfe-b79d-a4cd7c30491a';
